...
def search_products(query):
    ...
    results = data.sort_values(by=['similarity'], ascending=False).head(30)[['Title', 'Description', 'Category']]
    return results

# web app
img = Image.open('amazon_png.png')
st.image(img,width=200)
st.title("Search Engine and Product Recommendation System on a Data")
query = st.text_input("Enter Product Name")
sumbit = st.button('Search')
if sumbit:
    if(query==''):
        pag.alert(text="Please enter some product for searching recommendation", title="Blank data detected")# Message Box
    else:
...
<style>
.stApp {
  background-image: url("amazon_png.png");
